...
if [ ! -d "dlkcat/src/DeeplearningApproach/Data/input" ]; then
    (cd dlkcat/src/DeeplearningApproach/Data && unzip input.zip)
fi
...
mkdir -p unikp/UniKP
if [ ! -f "unikp/UniKP/UniKP for kcat.pkl" ]; then
    (cd unikp/UniKP && \
...
if [ ! -d "catpred/data/pretrained/production/kcat" ]; then
    if [ ! -f "catpred/capsule_data_update.tar.gz" ]; then
        echo "Downloading capsule_data_update.tar.gz (this may take a while)..."
        (cd catpred && wget -q --show-progress -c --tries=5 --timeout=30 https://catpred.s3.us-east-1.amazonaws.com/capsule_data_update.tar.gz || \
    ...
    (cd catpred && tar -xzf capsule_data_update.tar.gz && rm capsule_data_update.tar.gz)
fi
