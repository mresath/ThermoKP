...
def main() -> None:
    ...
    next_level = []
    if n_failed_benchmark > 0:
        next_level.append(("Benchmark Featurization Failures", n_failed_benchmark,
                      {"color": INK_MUTED, "label_pos": "top"}))
    next_level.append(("Benchmark Evaluated", benchmark_evaluated, {"color": GOOD, "label_pos": "top"}))
    
    if n_failed_benchmark > 0:
        flows.append(("Benchmark Holdout Table", "Benchmark Featurization Failures", n_failed_benchmark))
    flows.append(("Benchmark Holdout Table", "Benchmark Evaluated", benchmark_evaluated))
    ...
    if clean_total > 0:
        footnote_lines.append(
            ...
            f"thermodynamics {validator_kv.get('Dropped (Thermodynamics)', 0):,} · "
            f"chemicals {validator_kv.get('Dropped (Chemicals)', 0):,} · "
            f"structures {validator_kv.get('Dropped (Structures)', 0):,} · "
            f"sequence {validator_kv.get('Dropped (Sequence)', 0):,}"
        )
    footnote_lines.append(
        f"Validation failure logs: {n_failed_chem} unresolved chemical names · "
        f"{n_failed_seq} failed (UniProt, mutation) pairs · "
        f"{n_failed_struct} missing structures"
    )
    if n_failed_benchmark:
        ...
        footnote_lines.append(
            f"Benchmark Featurization Failures ({n_failed_benchmark:,}, "
            f"thermokp.py live inference): {tkp_breakdown}"
        )
    if tensorization_ran:
        footnote_lines.append(
            f"Tensor Build Failures ({n_failed_tensors:,}): mostly 3D-conformer/pocket-"
