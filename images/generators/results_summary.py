...
from ._style import AMBER, INK, INK_MUTED, INK_SECONDARY, MAGENTA, NAVY, SURFACE, TEAL, save
...
# ═══════════════════════════════════════════════════════════════════════════
#  Figure 1: parity plots, error concentration
# ═══════════════════════════════════════════════════════════════════════════
...
def generate_crossmodel_comparison(store: dict) -> None:
    ...
    None
    """
    models = ["pinn", "baseline", "no_mutant", "dlkcat", "unikp", "catpred"]
    ...
    for row, (param, symbol, color) in enumerate([("kcat", r"$k_{cat}$", NAVY), ("km", r"$K_m$", MAGENTA)]):
        ...
        for col, (key, m_label) in enumerate(metrics):
            ...
            wt_vals = [runs[m][param].get("wild_type", {}).get(key, float("nan")) for m in row_models]
            mut_vals = [runs[m][param].get("mutant", {}).get(key, float("nan")) for m in row_models]
            overall_vals = [runs[m][param].get("overall", {}).get(key, float("nan")) for m in row_models]

            bars_wt = ax.bar(x - bw, wt_vals, width=bw, color=NAVY, label="Wild-type")
            bars_mut = ax.bar(x, mut_vals, width=bw, color=MAGENTA, label="Mutant")
            bars_overall = ax.bar(x + bw, overall_vals, width=bw, color=TEAL, label="Overall")
            ...
            if row == 0 and col == 0:
                ax.legend(loc="best", fontsize=7.0, frameon=False)

            title = f"{symbol} {m_label}"
            ax.set_title(title, fontsize=10, color=INK_SECONDARY, loc="center")

            valid_vals = [v for v in wt_vals + mut_vals + overall_vals if not np.isnan(v)]
            ...
            for bar_set, val_set in [(bars_wt, wt_vals), (bars_mut, mut_vals), (bars_overall, overall_vals)]:
                for bar, v in zip(bar_set, val_set):
                    if np.isnan(v):
                        continue
                    ...
                    ax.text(bar.get_x() + bar.get_width() / 2, v + (headroom * 0.15 if v >= 0 else -headroom * 0.15),
                            format_str, ha="center", va=va, fontsize=6.5, color=INK, rotation=90)
...
def _r2_bar_panel(ax, val_run: dict, bench_run: dict, param: str, title: str, ylim: tuple) -> None:
    ...
    None
    """
    groups = ["Validation", "Benchmark"]
    runs = [val_run, bench_run]
    
    wt_r2 = [r[param].get("wild_type", {}).get("r2", float("nan")) for r in runs]
    mut_r2 = [r[param].get("mutant", {}).get("r2", float("nan")) for r in runs]
    overall_r2 = [r[param].get("overall", {}).get("r2", float("nan")) for r in runs]

    x = np.arange(len(groups))
    width = 0.25
    bars_wt = ax.bar(x - width, wt_r2, width, color=NAVY, label="Wild-Type")
    bars_mut = ax.bar(x, mut_r2, width, color=MAGENTA, label="Mutant")
    bars_overall = ax.bar(x + width, overall_r2, width, color=TEAL, label="Overall")

    lo, hi = ylim
    headroom = (hi - lo) * 0.06
    ax.set_ylim(lo, hi)
    for bars in (bars_wt, bars_mut, bars_overall):
        for bar in bars:
            v = bar.get_height()
            if np.isnan(v):
                continue
...
def generate_r2_comparison(store: dict, model_type: str) -> None:
    ...
    all_vals = [
        ...
        for run in (val_run, bench_run) for param in ("kcat", "km") for k in ("wild_type", "mutant", "overall")
    ]
