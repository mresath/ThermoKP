...
BENCHMARK_ENZYME_COUNT: int = 250
...
BENCHMARK_MIN_MUTANT_ENZYMES: int = 25
...
def _clean_and_aggregate_database(db_path: pathlib.Path = DEFAULT_DB_PATH) -> None:
    ...
    def filter_and_remove_outliers(g: pd.DataFrame) -> pd.DataFrame:
        """
        Filter outliers within large groups (>= 3 records) using a robust 
        Median Absolute Deviation (MAD) check, then reject any remaining 
        group where the max/min ratio exceeds VARIANCE_RATIO_THRESHOLD.
        """
        g_clean = g.copy()
        
        # 1. Filter extreme outliers if group is big enough
        for col in ["kcat", "km"]:
            vals = g_clean[col].dropna()
            if len(vals) >= 3:
                median = np.median(vals)
                mad = np.median(np.abs(vals - median))
                # Enforce a minimum deviation threshold (e.g. 20% of median) so tight clusters aren't shredded
                mad = max(mad, 0.2 * median)
                
                lower_bound = median - 3.0 * mad
                upper_bound = median + 3.0 * mad
                
                mask = g_clean[col].isna() | ((g_clean[col] >= lower_bound) & (g_clean[col] <= upper_bound))
                g_clean = g_clean[mask]

        # 2. Check remaining variance
        for col in ["kcat", "km"]:
            vals = g_clean[col].dropna()
            if len(vals) > 1:
                if vals.max() / (vals.min() + 1e-9) > VARIANCE_RATIO_THRESHOLD:
                    return pd.DataFrame()  # Reject the whole group
                    
        return g_clean

    df_filtered = grouped.apply(filter_and_remove_outliers).reset_index()
    # Drop the original index column inserted by reset_index()
    df_filtered = df_filtered.drop(columns=[c for c in df_filtered.columns if c.startswith("level_")])
    
    filtered_count = len(df_filtered)
    if filtered_count < cleaned_count:
        logger.info("Dropped %d records due to high intra-group variance (max/min > %.1fx) or outlier removal.",
