...
async def _process_database(limit: Optional[int] = None, start_entry: Optional[int] = None, accelerator: Optional[Accelerator] = None):
    ...
    total = len(df_params)
    logger.info(f"Assigned {total} targets to this process (Global: {global_total}).")
    ...
    async def process_entry(i, row):
        ...
        async with semaphore:
            logger.info(f"[{start_idx + i + 1}/{global_total}] Processing {uniprot_id} (Entry {entry_id})...")
            
            out_file = TENSOR_DIR / f"entry{entry_id}_{uniprot_id}.pt"
            if out_file.exists():
                logger.info(f"[{start_idx + i + 1}/{global_total}] Skipping {uniprot_id} (Entry {entry_id}): Tensor already exists.")
                successful += 1
                return
            ...
            try:
                ...
                logger.error(f"[{start_idx + i + 1}/{global_total}] Skipping {uniprot_id}: {e}")
