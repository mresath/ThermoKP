...
def generate_conformer(mol: Optional[Chem.Mol]) -> Optional[torch.Tensor]:
    ...
    # RDKit's ETKDG distance geometry fails with a hard C++ abort if the
    # molecule contains dummy/wildcard atoms (*). We make a copy and
    # temporarily treat them as Carbon atoms just to generate 3D coordinates.
    ...
    try:
        ...
        logger.warning(f"EmbedMolecule raised an exception: {e}. Trying random coords...")
        params.useRandomCoords = True
        try:
            if EmbedMolecule(mol_h, params) == -1:
                return None
