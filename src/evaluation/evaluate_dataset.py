...
def _get_training_sequences(model_type: str) -> Dict[str, str]:
    ...
    """
    if model_type not in ["pinn", "baseline", "no_mutant"]:
...
def _load_tensor_records(
    ...
    if split == "all":
        ...
        train_indices, val_indices, _ = train_val_split(dataset, train_frac=0.9, seed=42)
...
def main() -> None:
    ...
    parser.add_argument("--model_type", type=str, default="all", choices=["pinn", "baseline", "no_mutant", "thermokp", "dlkcat", "unikp", "catpred", "all"],
                        help="Model to evaluate. If not 'all' or 'thermokp', you must provide a valid --checkpoint path unless "
                             "using an external model. By default, it falls back to "
                             "models/best_model.pth, final_model.pth for --model_type pinn, "
                             "models/best_no_mutation_model.pth for no_mutant, and "
                             "models/best_baseline_model.pth for baseline.")
    ...
    if args.model_type == "all":
        model_types_to_run = ["pinn", "baseline", "no_mutant", "dlkcat", "unikp", "catpred"]
    elif args.model_type == "thermokp" or args.model_type == "both":
        model_types_to_run = ["pinn", "baseline", "no_mutant"]
    else:
    ...
    for model_type in model_types_to_run:
        ...
        model = None
        if model_type in ["pinn", "baseline", "no_mutant"]:
