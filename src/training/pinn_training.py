...
class PINNTrainer:
    ...
    def __init__(
        ...
        train_indices, val_indices, idx_to_key = train_val_split(dataset, train_frac=0.9, seed=42)
    ...
    def train(self) -> None:
        ...
        """
        if getattr(self, "model_suffix", ""):
            suffix = self.model_suffix
        else:
            suffix = "_baseline" if self.is_baseline else ""
            
        best_path = f"models/best{suffix}_model.pth"
        ...
        if self._pareto_checkpoint_saved:
            ...
            report_model.load_state_dict(torch.load(best_path, map_location=self.device, weights_only=True))
            report_model.eval()
