"""
===========================================================================
ThermoKP No-Mutation Baseline NN Training Script
Description: Trains a physics-informed model without mutation features
===========================================================================

Workflow:
1. Loads the same dataset from data/processed/tensors used by train.py.
2. Initializes NoMutationNNModel (which zeroes out mutation features).
3. Instantiates PINNTrainer and sets the model_suffix to '_no_mutation'.
4. Triggers the training loop.

Purely an ablation control: this model shares the exact same architecture
as ThermoKPModel (train.py) but discards the mutation feature vector by 
zeroing it out in the forward pass. This isolates the effect of the 
learned mutation embeddings on overall model performance.
...
#  Model Wrapper
# ═══════════════════════════════════════════════════════════════════════════
class NoMutationNNModel(nn.Module):
    """
    ...
    def forward(self, data, return_components: bool = False) -> tuple[torch.Tensor, ...]:
        ...
        if hasattr(data, 'mutation_features'):
            # Ablate mutation features by zeroing them out
            data.mutation_features = torch.zeros_like(data.mutation_features)
...
def main() -> None:
    """
    Main entry point for the ThermoKP No-Mutation Training Script.
    Parses arguments, initializes datasets and models, and starts training.
    """

    parser = argparse.ArgumentParser(description="ThermoKP No-Mutation Training Script")
    ...
    logger.info("=======================================================================")
    logger.info("Starting ThermoKP No-Mutation Training Script")
    logger.info("=======================================================================")
    ...
    # 2. Model
    logger.info("Initializing No-Mutation Model...")
    model = NoMutationNNModel(
    ...
    trainer.model_suffix = "_no_mutation"
    trainer.train()
